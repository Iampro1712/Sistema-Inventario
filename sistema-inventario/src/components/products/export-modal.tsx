  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (selectedFormat === 'csv') {
        response = await fetch('/api/productos/export');
      } else {
        response = await fetch('/api/productos/export', {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        ...
        let filename = `productos_${new Date().toISOString().split('T')[0]}.${selectedFormat}`;
        ...
        alert(error.error || 'Error al exportar productos');
      }
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[400px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Descarga todos los productos en el formato seleccionado.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-muted p-3 rounded-lg">
            ...
            <ul className="text-xs text-muted-foreground space-y-1">
              <li>• Datos básicos (nombre, SKU, descripción)</li>
              ...
              {selectedFormat === 'json' && (
                <>
                  <li>• Margen de ganancia calculado</li>
                  <li>• Valor total en stock</li>
